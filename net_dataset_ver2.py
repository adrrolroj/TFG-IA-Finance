...
class Net_dataset_ver2(Dataset):
    ...
    def __len__(self):
        return len(self.data_close) - (self.number_training * 6 + 1)

    def __getitem__(self, idx):
        torch.set_printoptions(precision=15)
        size = self.number_training * 6 + 1
        x_train = []
        y_train = []
        z_train = []
        sal_train = []
        ...
        for j in range(idx, idx + size):
            ...
            if len(x_train) != self.number_training:
                if count > self.number_training * 5:
                    percent = limit_one((self.data_close[j] / self.data_close[j - 1] - 1) * 100)
                    x_train.append(percent)
                if count > self.number_training * 3 and count % 3 == 0:
                    percent = limit_one((self.data_close[j] / self.data_close[j - 3] - 1) * 100)
                    y_train.append(percent)
                if count % 6 == 0:
                    percent = limit_one((self.data_close[j] / self.data_close[j - 6] - 1) * 100)
